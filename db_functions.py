...
def get_final_sum_for_user(id_user):
    ...
    :return: result:int
    """
    result = cursor.execute("SELECT task_1, task_2, task_3, task_4, task_5, task_6, task_7, task_8 FROM Scores WHERE ID_user = ? AND time = (SELECT max(time) FROM Scores WHERE ID_user = ?)", [id_user]).fetchall()
    if len(result) == 0:
        result = 0
    else:
        result = sum(result[0])
